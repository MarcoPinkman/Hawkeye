...
QWEN_RESP_FORMAT_PROMPT = (
    """The Schema of Response JSON is: {"schema": {"type": "object", "properties": {"events": {"type": "array", "items": {"type": "object", "properties": {"event_code": {"type": "string"}, "detected": {"type": "boolean"}, "explanation": {"type": "string"}}, "required": ["event_code", "detected", "explanation"]}}}, "required": ["events"]}}"""
)
...
class VideoEventDetector:
    ...
    def _build_system_message(self, context: str) -> Dict[str, str]:
        ...
        return {
            "role": "system",
            "content": ' '.join([self.system_prompt.format(context=context), QWEN_RESP_FORMAT_PROMPT])
        }
    ...
    def _build_user_message(
        ...
        for frame in frames:
            video_content["video"].append(frame)
        ...
        try:
            ...
            if not os.path.exists("messages.json"):
                with open("messages.json", "w", encoding="utf-8") as f:
                    json.dump([system_message, user_message], f)

            response = self.client.chat.completions.create(
                model=self.model,
                messages=[system_message, user_message],
                # response_format={"type": "json_schema", "json_schema": json_schema},
                response_format = {"type":"json_object"} # for qwen-vl-max
            )
